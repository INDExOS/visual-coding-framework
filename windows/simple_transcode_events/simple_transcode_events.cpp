...
// VCF Graph level callbacks
//
class MyRTCallbacks : public VCFAPI_Callback
{
public:
    void SystemConfig(const char* component, const char* name, const char* value, const char* attribute)
    {
        // Not implemented since we do not subscribe for this event
    }

    void Status(const char* status, const int res)
    {
        printf("Status : %s\n", status);
    }
};
